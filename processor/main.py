...
def _process_pending_files(engine: QCEngine, logger: logging.Logger, mtime_tracker: dict):
    from .file_funcs import list_raw_files
...
# we can use this to test the uploads folder 
def main():
    #args = parse_args()
    # lets put this in logs directory 
    #log_path = os.path.join(args.output, "watchdog.log")
    log_path = os.path.join("../logs", "watchdog.log")
    logger = build_logger(log_path)
    engine = QCEngine(config_path="dq_master.yaml", logger=logger)
